...
~/.tessera store,                release-key host, timestamp     directories, CDNs,
tessera binary                   signer, log server, origin      DNS, network path,
                                                                 other consumers
publisher offline
root environment
```
...
Principles: fail closed, attribute evidence, never degrade to unverified. There is no override flag; the only remediations are trust-state operations (`tessera trust add`, updating pins, publisher-side re-signing), never verification bypasses.
