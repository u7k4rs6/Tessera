...
The surface is a single binary-style CLI, `tessera`. Justification against the users and the threat model rather than taste:
...
A read-only web viewer for provenance graphs is a plausible later addition and is explicitly out of scope for v1; `tessera provenance --dot` exports Graphviz for anyone who wants a picture today.
...
```
$ tessera keygen --role root --out /media/airgap/acme-root.key      # offline machine
new root key: b3:9f2a11c4...  (fingerprint to distribute out-of-band)

$ tessera publisher init acme-lab --root-pub acme-root.pub
$ tessera keygen --role release
$ tessera keygen --role timestamp
$ tessera publisher delegate --role release --key rk-2026-07.pub    # prints root-doc
$ tessera publisher delegate --role timestamp --key tk-2026-07.pub  # update to sign
                                                                    # on the offline
                                                                    # machine, then import
```
...
```
$ tessera trust add acme-lab b3:9f2a11c4... \
...
materialized ~/.tessera/verified/acme-lab/bert-tiny/1.2.0
$ echo $?
0
```
...
```
$ tessera verify ./weights.safetensors --ref acme-lab/bert-tiny@1.2.0
manifest   ok  b3:77e1... (cached)
content    FAIL file digest b3:12f9... expected b3:77e1... (first bad chunk: 87)
result: NOT the signed artifact. evidence: ~/.tessera/quarantine/20260713T0912-40/
$ echo $?
40
```
...
```
$ tessera provenance acme-lab/bert-tiny@1.2.0
...
$ tessera diff acme-lab/sst5@1.4.1 acme-lab/sst5@1.4.2
...
`tessera status` summarizes trust state per pinned publisher: root version, timestamp age, checkpoint size, count of verified artifacts, and any artifacts flagged `revoked` pending re-signed manifests.

## 6. Mirror operator experience

```
$ tessera mirror sync acme-lab --from https://origin.acme.dev:7433 --store /srv/tessera
...
$ tessera mirror serve --store /srv/tessera --bind 0.0.0.0:7433
...
evidence:  ~/.tessera/quarantine/20260713T0931-40/
remedy:    add an honest mirror (tessera trust add ... --mirror URL) or retry later
$? = 40
```
...
remedy:    add an independent mirror; do not use the stale copy, tessera will not
$? = 30
```
...
remedy:    obtain the fingerprint from an out-of-band channel, then
           tessera trust add acme-lab <fingerprint>
$? = 43
```
...
Environment: `TESSERA_HOME` relocates the store; `NO_COLOR` respected. State paths are the storage layout from the architecture document; the config file never contains key material or verification toggles.
